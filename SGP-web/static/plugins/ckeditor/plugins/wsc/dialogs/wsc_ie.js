...
CKEDITOR.dialog.add('checkspell', function (a) {
  function c (a, c) { var d = 0; return function () { typeof window.doSpell === 'function' ? (typeof e !== 'undefined' && window.clearInterval(e), l(a)) : d++ == 180 && window._cancelOnError(c) } } function l (c) {
    var f = new window._SP_FCK_LangCompare(), b = CKEDITOR.getUrl(a.plugins.wsc.path + 'dialogs/'), e = b + 'tmpFrameset.html'; window.gFCKPluginName = 'wsc'; f.setDefaulLangCode(a.config.defaultLanguage); window.doSpell({ctrl: g,
...
function (c) { if (typeof window.WSC_Error === 'undefined') { CKEDITOR.document.getById(d).setStyle('display', 'none'); var b = CKEDITOR.document.getById(h); b.setStyle('display', 'block'); b.setHtml(c || a.lang.wsc.notAvailable) } }; return {title: a.config.wsc_dialogTitle || a.lang.wsc.title,
  minWidth: 485,
  minHeight: 380,
  buttons: [CKEDITOR.dialog.cancelButton],
  onShow: function () {
    var b = this.getContentElement('general', 'content').getElement(); b.setHtml(m); b.getChild(2).setStyle('height', this._.contentSize.height + 'px'); typeof window.doSpell !==
...
CKEDITOR.dialog.on('resize', function (a) { a = a.data; var c = a.dialog; c._.name == 'checkspell' && ((c = (c = c.getContentElement('general', 'content').getElement()) && c.getChild(2)) && c.setSize('height', a.height), c && c.setSize('width', a.width)) })
