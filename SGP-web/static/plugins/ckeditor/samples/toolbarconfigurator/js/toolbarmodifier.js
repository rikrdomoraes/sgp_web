...
{text: 'Add row separator', group: 'edit', position: 'left', cssClass: 'button-a-soft', clickCallback: function () { this._addSeparator() }}, {text: 'Select config', group: 'config', position: 'left', cssClass: 'button-a-soft', clickCallback: function () { this.configContainer.findOne('textarea').$.select() }}, {text: 'Back to configurator',
  group: 'config',
  position: 'right',
        ...
        var h = b[m]; if (h === '/')f.push("'/'"); else {
          if (e) {
            for (var k = h.groups.length; k--;) {
              d.getTotalSubGroupButtonsNumber(h.groups[k], this.fullToolbarEditor) === 0 && h.groups.splice(k,
1)
            }
          } e && h.groups.length === 0 || f.push(l.stringifyJSONintoOneLine(h, {addSpaces: !0, noQuotesOnKey: !0, singleQuotes: !0}))
        }
      }b = '\n\t\t' + f.join(',\n\t\t')
    ...
    var c = this.modifyContainer.find('li[data-type\x3d"group"]'); this.modifyContainer.on('mouseleave', function () { this._dehighlightActiveToolGroup() }, this); for (var e = c.count(), f = 0; f < e; f += 1)c.getItem(f).on('mouseenter', a); CKEDITOR.document.on('keypress', function (a) { a = a.data.$.keyCode; a = a === 32 || a === 13; var c = new CKEDITOR.dom.element(CKEDITOR.document.$.activeElement); c.getAscendant(function (a) { return a.$ === b.mainContainer.$ }) && a && c.data('type') === 'button' && c.findOne('input').$.click() }); this.modifyContainer.on('click',
function (a) { var c = a.data.$, e = new CKEDITOR.dom.element(c.target || c.srcElement); if (a = d.getGroupOrSeparatorLiAncestor(e)) { b.cachedActiveElement = document.activeElement; if (e.$ instanceof HTMLInputElement)b._handleCheckboxClicked(e); else if (e.$ instanceof HTMLButtonElement && (c.preventDefault ? c.preventDefault() : c.returnValue = !1, (c = b._handleAnchorClicked(e.$)) && c.action == 'remove')) return; c = a.data('type'); a = a.data('name'); b._setActiveElement(c, a); b.cachedActiveElement && b.cachedActiveElement.focus() } }); this.toolbarContainer ||
(this._createToolbar(), this.toolbarContainer.insertBefore(this.mainContainer.getChildren().getItem(0))); this.showToolbarBtnsByGroupName('edit'); this.configContainer || (this.configContainer = new CKEDITOR.dom.element('div'), this.configContainer.addClass('configContainer'), this.configContainer.addClass('hidden'), this.mainContainer.append(this.configContainer)); return this.mainContainer
  ...
  }; d.parseGroupToConfigValue = function (a) { if (a.type == 'separator') return '/'; var b = a.groups, c = b.length; delete a.totalBtns; for (var e = 0; e < c; e += 1)b[e] = b[e].name; return a }; d.getGroupOrSeparatorLiAncestor = function (a) { return a.$ instanceof HTMLLIElement && a.data('type') == 'group' ? a : d.getFirstAncestor(a, function (a) { a = a.data('type'); return a == 'group' || a == 'separator' }) }; d.prototype._setActiveElement = function (a,
b) { this.currentActive && this.currentActive.elem.removeClass('active'); if (a === null) this._dehighlightActiveToolGroup(), this.currentActive = null; else { var c = this.mainContainer.findOne('ul[data-type\x3dtable-body] li[data-type\x3d"' + a + '"][data-name\x3d"' + b + '"]'); c.addClass('active'); this.currentActive = {type: a, name: b, elem: c}; a == 'group' && this._highlightGroup(b); a == 'separator' && this._dehighlightActiveToolGroup() } }; d.prototype.getActiveToolGroup = function () {
...
}; d.prototype.getFirstEnabledButtonInGroup = function (a) { var b = this.actualConfig.toolbarGroups; a = this.getGroupIndex(a); b = b[a]; if (a === -1) return null; a = b.groups ? b.groups.length : 0; for (var c = 0; c < a; c += 1) { var e = this.getFirstEnabledButtonInSubgroup(b.groups[c].name); if (e) return e } return null }; d.prototype.getFirstEnabledButtonInSubgroup = function (a) {
...
: 0, c = 0; c < b; c += 1) { var e = a[c].name; if (!this.isButtonRemoved(e)) return e } return null
  ...
  a = new CKEDITOR.dom.element(a); var b = a.getAscendant('li'), c = b.getAscendant('ul'), e = b.data('type'), d = b.data('name'), g = a.data('direction'), m = g === 'up' ? b.getPrevious() : b.getNext(), h; if (a.hasClass('disabled')) return null
  if (a.hasClass('remove')) return b.remove(), this._removeSeparator(b.data('name')), this._setActiveElement(null), {action: 'remove'}; if (!a.hasClass('move') || !m) return {action: null}; if (e === 'group' || e === 'separator')h = this._moveGroup(g, d); e === 'subgroup' && (h = b.getAscendant('li').data('name'), h = this._moveSubgroup(g, h, d)); g === 'up' && b.insertBefore(c.getChild(h)); g === 'down' && b.insertAfter(c.getChild(h)); for (var k; b = g === 'up' ? b.getPrevious() : b.getNext();) if (this.emptyVisible || !b.hasClass('empty')) { k = b; break }k || (k =
'[data-direction\x3d"' + (g === 'up' ? 'down' : 'up') + '"]', this.cachedActiveElement = a.getParent().findOne(k)); this._refreshMoveBtnsAvalibility(); this._refreshBtnTabIndexes(); return {action: 'move'}
...
: (c = a.getFirst(b), a = a.getLast(b)); if (c) var e = c.findOne('p button[data-direction\x3d"up"]'); if (a) var d = a.findOne('p button[data-direction\x3d"down"]'); e && (e.addClass('disabled'), e.setAttribute('tabindex', '-1')); d && (d.addClass('disabled'), d.setAttribute('tabindex', '-1'))
  ...
  var b = CKEDITOR.tools.indexOf(this.actualConfig.toolbarGroups, function (b) { return b.type == 'separator' && b.name == a }); this.actualConfig.toolbarGroups.splice(b,
1); this._refreshMoveBtnsAvalibility(); this._refreshBtnTabIndexes(); this._refreshEditor()
}; d.prototype._determineSeparatorToAddIndex = function () { return this.currentActive ? (this.currentActive.elem.data('type') == 'group' || this.currentActive.elem.data('type') == 'separator' ? this.currentActive.elem : this.currentActive.elem.getAscendant('li')).getIndex() : 0 }; d.prototype._moveElement = function (a, b, c) {
  function e (a) { return a.totalBtns || a.type == 'separator' }c = this.emptyVisible ? c == 'down' ? b + 1 : b - 1 : d.getFirstElementIndexWith(a,
...
function () { for (var a = this.actualConfig.toolbarGroups, b = a.length; b--;) { var c = a[b], e = d.getTotalGroupButtonsNumber(c, this.fullToolbarEditor); c.type != 'separator' && (c.totalBtns = e) } }; d.prototype._addButtonToRemoved = function (a) { if (CKEDITOR.tools.indexOf(this.removedButtons, a) != -1) throw 'Button already added to removed'; this.removedButtons.push(a); this.actualConfig.removeButtons = this.removedButtons.join(','); this._refreshEditor() }; d.prototype._removeButtonFromRemoved = function (a) {
  a = CKEDITOR.tools.indexOf(this.removedButtons,
a); if (a === -1) throw 'Trying to remove button from removed, but not found'; this.removedButtons.splice(a, 1); this.actualConfig.removeButtons = this.removedButtons.join(','); this._refreshEditor()
}; d.parseGroupToConfigValue = function (a) { if (a.type == 'separator') return '/'; var b = a.groups, c = b.length; delete a.totalBtns; for (var d = 0; d < c; d += 1)b[d] = b[d].name; return a }; d.getGroupOrSeparatorLiAncestor = function (a) {
  return a.$ instanceof HTMLLIElement && a.data('type') == 'group' ? a : d.getFirstAncestor(a, function (a) {
    a = a.data('type')
    return a == 'group' || a == 'separator'
  })
}; d.createSeparatorLiteral = function () { return {type: 'separator', name: 'separator' + CKEDITOR.tools.getNextNumber()} }; d.prototype._toolbarConfigToListString = function () { for (var a = this.actualConfig.toolbarGroups || [], b = '\x3cul data-type\x3d"table-body"\x3e', c = a.length, e = 0; e < c; e += 1) var f = a[e], b = f.type === 'separator' ? b + d.getToolbarSeparatorString(f) : b + this._getToolbarGroupString(f); b += '\x3c/ul\x3e'; return d.getToolbarHeaderString() + b }; d.prototype._getToolbarGroupString = function (a) {
  var b =
...
? '' : 'checked\x3d"checked"'; return ['\x3cli data-tab\x3d"true" data-type\x3d"button" data-name\x3d"', this._getConfigButtonName(a.name), '"\x3e\x3clabel title\x3d"', a.label, '" \x3e\x3cinput tabindex\x3d"-1"type\x3d"checkbox"', b, '/\x3e', a.$.getOuterHtml(), '\x3c/label\x3e\x3c/li\x3e'].join('')
  ...
  a = a.name ? a.name : a; return ['\x3cp\x3e\x3cspan\x3e\x3cbutton title\x3d"Move element upward" data-tab\x3d"true" data-direction\x3d"up" class\x3d"move icon-up-big"\x3e\x3c/button\x3e\x3cbutton title\x3d"Move element downward" data-tab\x3d"true" data-direction\x3d"down" class\x3d"move icon-down-big"\x3e\x3c/button\x3e',
    a == 'row separator' ? '\x3cbutton title\x3d"Remove element" data-tab\x3d"true" class\x3d"remove icon-trash"\x3e\x3c/button\x3e' : '', a, '\x3c/span\x3e\x3c/p\x3e'].join('')
}; d.evaluateToolbarGroupsConfig = function (a) { return a = (function (a) { var c = {}, d; try { d = eval('(' + a + ')') } catch (f) { try { d = eval(a) } catch (g) { return null } } return c.toolbarGroups && typeof c.toolbarGroups.length === 'number' ? JSON.stringify(c) : d && typeof d.length === 'number' ? JSON.stringify({toolbarGroups: d}) : d && d.toolbarGroups ? JSON.stringify(d) : null }(a)) }
  return d
})()
