  ...
  <div>
    ...
    <div v-if="!showGrid" style="padding-left:0" class="row well well-sm no-shadow animated fadeInDownOutUp">
      <div class="col-md-16">
          ...
          <div class="col-md-16">
            <div class="col-md-6 col-lg-4">
              <input
                ...
                ref="Vigencia"
                name="Vigencia"
                v-model="documento.dt_data_vigencia"
                class="form-control"
                v-mask="'##/##/####'"
                data-vv-as="Vigencia"
                :masked="masked"
                v-validate="{required: (documento.tipo_documento === undefined ? false : !!documento.tipo_documento.vigencia_obrigatoria), validdate: true}"                
              >
        ...
        <div class="form-group" style="margin-bottom:0">
          <div class="col-md-9 col-lg-10 col-md-offset-3">
            <button @click="adicionaDocumento" type="button"
                    class="btn btn-confirm btn-flat" id="cmpt-upload-add">
              <i class="fa fa-check" aria-hidden="true"></i> 
              <span v-if="editando">Ok</span>
              <span v-else>Adicionar Documento</span>
            </button>
            <button @click="onCancel" type="button"
                    class="btn btn-back btn-flat" id="cmpt-upload-cancel">
    ...
    <div v-if="showGrid">
      ...
      <div class="row" v-if="!disable">
        <span v-if="totalMb">&nbsp&nbsp&nbsp&nbsp&nbsp<b>{{totalMb}}</b> MB de <b>80</b> MB permitidos </span>
        <div class="">
          <div class="col-md-4">
              <div v-if="totalMb < 50" class="bar" :style="'height:15px;background:green;width:' + progress + '%'"></div>
              <div v-if="totalMb > 50 && totalMb < 70" class="bar" :style="'height:15px;background:orange;width:' + progress + '%'"></div>
              <div v-if="totalMb > 70" class="bar" :style="'height:15px;background:red;width:' + progress + '%'"></div>
          </div>
          <span v-if="totalMb">{{progress}} % ({{contador}} arquivo(s) novo(s))</span>
        </div>
        <br>
        <div class="col-sm-12" v-if="faseProposta !== 'FORMALIZACAO' && faseProposta !== 'APROVACAO_PROJETO' && faseProposta !== 'APROVACAO'">
          ...
          <div class="table-responsive">
            <table class="table table-condensed table-bordered table-striped">
              ...
              <tr v-for="pendente in documentTypes" v-if="!existeDocumento(pendente)" style="text-align: center"
                ...
                <td>-</td>
                <td align="center">
                  <button type="button" style="max-width: 135px; text-align:center;" @click="onAddFile(pendente)"
                          class="btn btn-primary btn-sm btn-flat btn-block" v-if="!disable"><i class="fa fa-plus-circle"
                                                                                               aria-hidden="true"></i>
                    Adicionar arquivo
                  </button>
                </td>
              ...
              <tr v-for="(documento, indexDocumento) in propDocumentos" :value="documento" :key="indexDocumento"
                ...
                <td v-else> </td>
                <td v-if="localUso === 'PROPOSTA_COMPLETA_DOCUMENTOS_ANEXOS'">
                  <a v-if="documento.id || documento.file" @click="onDownload(documento)" href="javascript:void(0)" class="ng-binding"><i
                  class="fa fa-download" aria-hidden="true"></i> {{documento.ds_nome_arquivo}}</a>
                  <span v-else title="Erro no upload, envie o arquivo novamente">
                    <span>
                      {{documento.ds_nome_arquivo}} 
                    </span>
                    <span style="color: #FF9900">
                      <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>  Erro no upload do arquivo </span>
                    </span> 
                ...
                <td v-if="!documento.dt_data_validade">
                  {{documento.dt_data_vigencia | moment('DD/MM/YYYY')}}
                </td>
                <td v-else>{{documento.dt_data_validade  | moment('DD/MM/YYYY')}}</td>
                <td v-if="!disable">
                  <button v-if="documento.id && !disable && !PropProrrogacaoEdit" v-show="faseProposta !== 'APROVACAO_PROJETO' && faseProposta !== 'APROVACAO'" type="button" @click="editaDocumento(documento, indexDocumento)"
                          class="btn btn-primary btn-sm btn-flat"><i class="fa fa-pencil-square-o"
                                                                     aria-hidden="true"></i> Editar
                  </button>
                  <!-- // 66888 - Desativar exclusão de anexo para Alterações no Projeto/Demais Ajustes -->
                  <button v-if="!disable && !PropEditandoAlteracaoProjeto" type="button" v-show="faseProposta !== 'APROVACAO_PROJETO' && faseProposta !== 'APROVACAO'"
                          v-confirm="{ok: retorno => deletaDocumento(indexDocumento), cancel: doNothing,
                           message: 'Deseja <b>APAGAR</b> a documento ' + documento.ds_nome_arquivo + ' ?'}"
                          class="btn btn-danger btn-sm btn-flat"><i class="fa fa-trash" aria-hidden="true"></i> Excluir
                  </button>
                  <div class="col-sm-offset-1 fa-lg">
                    <span v-if="_.hasIn(atualizados, indexDocumento)"
...
<script>
  ...
  export default {
    name: 'upload-documentos-php-novo',
    data () {
      return {
        showGrid: true,
        tamanho: 0,
    ...
    props: ['propDocumentos', 'documentTypes', 'documentosObrigatorios', 'disable', 'localUso', 'mudaDocumento', 'diferenca', 'analise', 'revisao', 'salvouOK', 'propsEtapa', 'faseProposta', 'editandoAlteracaoProjeto', 'editandoProrrogacao'],
    watch: {
