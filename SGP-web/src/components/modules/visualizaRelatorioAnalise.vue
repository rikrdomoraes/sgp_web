<template>
  ...
  <div class="content-wrapper" style="min-height: 870px;">
    ...
    <section class="content-header">
      <h1>Visualizar Relatório de Análise
      ...
      <div class="column all-centered etapa-carga-log-button" @click="mostrarLogCarga = true" :title="'<h5>Verificar Log de Carregamento</h5>'" v-tippy="{ inertia : true, arrow : true,  placement : 'right', animation : 'perspective', distance:'1', size: 'large', arrow : true }">
        <i class="fa fa-warning"></i>
      </div>
    </section>
    
    
    <div class="column centered-start etapa-carga-container" v-if="mostrarLogCarga">
      <div class="column all-centered etapa-aviso-container" v-if="mostrarAviso">
        <div>
           <i class="fa fa-warning"> </i> AVISO: Existem dados que não foram carregados para o relatório de análise conforme log abaixo. O que pode ocorrer nos casos em que a proposta ainda não possui projeto vinculado ou está com seu preenchimento incompleto. Revise o Log e caso necessário, entre em contato com o administrador do sistema ou recarregue o relatório.
        </div>
        <div class="row-flex start-centered etapa-aviso-button-container">
          <div class="row-flex all-centered etapa-button-ok" @click="mostrarLogCarga = false">
            Estou Ciente, continuar.
          </div>
          <div class="row-flex all-centered etapa-button-reload" @click="recarregarRelatorio()">
            Recarregar Relatório.
          </div>
        </div>
      </div>
      <div class="column centered-start etapa-carga-content">
        <div class="column all-end etapa-close-container">
          <i class="fa fa-close etapa-close-button" @click="mostrarLogCarga = false"></i>
        </div>
        <div class="row-flex centered-start etapa-carga-titulo"><i class="fa fa-user-secret"></i> Status de Carga do Relatório de Análise</div>
        <div class="column all-centered etapa-carga-info" v-for="(etapa, index) in etapaCarga">
          <div class="row-flex centered-between etapa-carga-row">
            <div class="row-flex etapa-carga-nome">
              {{index + 1}} - {{ etapa.nome }}
            </div>
            <div class="row-flex all-centered etapa-carga-status" :class="etapa.status ===  'OK' ? 'status-ok' : etapa.status === 'Carregando' ? 'status-carregando' : 'status-erro'">
              {{ etapa.status }} <i class="fa fa-refresh ga-spin" v-if="etapa.status === 'Carregando'"></i> <i class="fa fa-check-circle" v-if="etapa.status === 'OK'"></i> <i class="fa fa-warning" v-if="etapa.status === 'ERRO'"></i>
            </div>
          </div>
          <div v-if="etapa.erro" class="column start-centered etapa-carga-erro-container">
            <div v-if="etapa.erro.response">
              {{ etapa.nome }} | {{ etapa.erro.response.data.message }}
            </div>
            <div v-else>
              {{ etapa.nome }} | {{ etapa.erro }}
            </div>
          </div>
        </div>
      </div>
            ...
            <fieldset :disabled="proposta.ds_fase !== 'ANALISE'">
              ...
              <div class="box box-primary">
                <div class="box-header">
                    ...
                    <tr>
                      ...
                      <td class="w50">
                        <h4 class="titulo ressaltar">
                          ...
                          <span v-if="ehVoluntariado">Alçada do Comitê de Desenvolvimento Social</span>
                  ...
                  <div v-if="ehSimplificada">
                    <div class="titulo nao_imprimir alertaCamposPendentes">Modelo de Relatório de Análise para projetos até R$ 300.000,00</div>
                  </div>
                  ...
                  <div class="box-body">
                    ...
                    <div>
                      <table class="table table-condensed table-bordered">
                        ...
                        <!-- <tr>
                          <td class="cabecalho w30">Risco Calculado</td>
                          <td v-if='riscoCalculado' class="ressaltar" colspan="3">{{riscoCalculado}}</td>
                            <td class="ressaltar" colspan="3">{{riscoCalculado}}</td>
                          <td v-else class="ressaltar" colspan="3">Risco não calculado</td>
                        </tr> -->
                        <tr>
...
<!-- Municípios --------------------------------------------------------------------------------------------->
                  <div class="box-body">
                    <table class="table table-condensed table-bordered" v-if="proposta.municipalizacao">
                      ...
                      <tr>
                        <td colspan="2">
                            <span v-for="(municipio, key, index) in municipios = _.groupBy(proposta.municipalizacao, function (m) { return m.ds_municipio + '/' + m.ds_uf })">
                  ...
                  <div class="box-body" v-if="ehPIS">
                    ...
                    <div class="required" :class="{ 'has-error' : errors.first('apresentacao_entidade')}">
                      <div class="nao_imprimir">
                        <textarea-autosize :readonly="somenteLeitura" placeholder="" v-model="relatorioAnalise.apresentacao_entidade" name="ds_problemas" v-resizable @input="resizable" class="form-control"></textarea-autosize>
                      </div>
...
<!-- Problemas --------------------------------------------------------------------------------------------->
                  <div class="box-body" v-if="_.includes([2, 3], faixaDoProjeto.faixa) || ehPIS || ehVoluntariado">
                    ...
                    <div class="required" :class="{ 'has-error' : errors.first('ds_problemas')}">
                      <div class="nao_imprimir">
                        <textarea-autosize :readonly="somenteLeitura" placeholder="De forma sucinta, descreva a(s) situação(ões)-problema destacada(s) pela proponente." v-model="relatorioAnalise.ds_problemas" name="ds_problemas" v-resizable @input="resizable" class="form-control"></textarea-autosize>
                      </div>
...
<!-- Solução problemas --------------------------------------------------------------------------------------------->
                  <div class="box-body" v-if="_.includes([2, 3], faixaDoProjeto.faixa) || ehPIS || ehVoluntariado">
                    ...
                    <div class="required" :class="{ 'has-error' : errors.first('ds_solucao_problemas')}">
                      <div class="nao_imprimir">
                        <textarea-autosize :readonly="somenteLeitura" @input="resizable"  placeholder="De forma sucinta, descreva o que a instituição propõe executar para solucionar o(s) problema(s) e os gastos previstos."   v-model="relatorioAnalise.ds_solucao_problemas" name="ds_solucao_problemas" v-resizable class="form-control"></textarea-autosize>
                      </div>
...
<!-- Operacionalização --------------------------------------------------------------------------------------------->
                  <div class="box-body" v-if="ehPIS">
                    ...
                    <div class="required" :class="{ 'has-error' : errors.first('operacionalizacao_atividades')}">
                      <div class="nao_imprimir">
                        <textarea-autosize :readonly="somenteLeitura" placeholder="" v-model="relatorioAnalise.operacionalizacao_atividades" name="ds_problemas" v-resizable @input="resizable" class="form-control"></textarea-autosize>
                      </div>
...
<!-- Resultados esperados --------------------------------------------------------------------------------------------->
                  <div class="box-body" v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehVoluntariado && !ehSimplificada">
...
<!-- Aspectos positivos --------------------------------------------------------------------------------------------->
                  <div class="box-body" v-if="_.includes([2, 3], faixaDoProjeto.faixa) && (!ehPIS && !ehVoluntariado && !ehSimplificada)">
                    ...
                    <div>
                      <table class="table table-condensed table-bordered m0">
                        ...
                        <tr>
                          ...
                          <!-- Demanda 68888: adiciona jovens e mulheres no detalhe do público alvo -->
                  ...
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_indicadores_objetivo_geral" class="form-control" name="ds_indicadores_objetivo_geral">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_indicadores_objetivo_especifico" class="form-control" name="ds_indicadores_objetivo_especifico">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_indicadores_objetivo_compativel" class="form-control" name="ds_indicadores_objetivo_compativel">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="!ehPIS && !ehVoluntariado">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_indicadores_objetivo_estruturado" class="form-control" name="ds_indicadores_objetivo_estruturado">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="ehPIS || ehVoluntariado">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.existe_critica_objetivos_metas_indicadores" class="form-control" name="existe_critica_objetivos_metas_indicadores">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="(_.has(relatorioAnalise, 'existe_critica_objetivos_metas_indicadores') && relatorioAnalise.existe_critica_objetivos_metas_indicadores === 'Sim')">
                        <td colspan="2">
                          <div class="nao_imprimir">
                            <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.critica_objetivos_metas_indicadores" rows="7" v-resizable name="critica_objetivos_metas_indicadores" class="form-control"></textarea-autosize>
                          </div>
                      ...
                      <tr v-if="!ehPIS && !ehVoluntariado">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_indicadores_objetivo_estrategico" class="form-control" name="ds_indicadores_objetivo_estrategico">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
...
<!-- Objetivo Geral--------------------------------------------------------------------------------------------->
                  <div class="box-body">
                    ...
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-resizable data-vv-as="Objetivo Geral do Projeto" v-model="proposta.ds_objetivo_geral" name="objetivo Geral" class="form-control" rows="4" :disabled="true"></textarea-autosize>
                    </div>
...
<!-- Indicadores de Resultado --------------------------------------------------------------------------------------------->
                  <div class="box-body">
                    <div>
                      ...
                      <div class="nao_imprimir">
                        <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-resizable data-vv-as="Indicadores" v-model="proposta.ds_indicadores_resultado" name="indicadores" class="form-control" rows="4" :disabled="true"></textarea-autosize>
                      </div>
...
<!-- Divulgação do projeto --------------------------------------------------------------------------------------------->
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      <tbody>
                      <tr v-if="ehSimplificada">
                        <td>Os recursos para realização das ações contidas no Plano de Comunicação foram previstos na memoria de cálculo?</td>
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_acoes_comunicacao_memoria_calculo" class="form-control" name="ds_divulgacao_identificacao">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="!ehSimplificada">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_divulgacao_banner" class="form-control" name="ds_divulgacao_banner">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehSimplificada">
                        <td>Foi prevista a identificação de veículos a serem adquiridos com recursos financeiros do projeto (se for o caso) ?</td>
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_divulgacao_identificacao" class="form-control" name="ds_divulgacao_identificacao">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehSimplificada">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_divulgacao_placa" class="form-control" name="ds_divulgacao_placa">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehSimplificada">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_divulgacao_solenizacao" class="form-control" name="ds_divulgacao_solenizacao">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                  ...
                  <div class="box-body">
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_itens" class="form-control" name="ds_valores_itens">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_contrapartida" class="form-control" name="ds_valores_contrapartida">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="!ehPIS && !ehSimplificada">
                        <td>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_deacordo" class="form-control" name="ds_valores_deacordo">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehSimplificada">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_enquadrados" class="form-control" name="ds_valores_enquadrados">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa)">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_vedacoes" class="form-control" name="ds_valores_vedacoes">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2], faixaDoProjeto.faixa)">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_industrial" class="form-control" name="ds_valores_industrial">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2], faixaDoProjeto.faixa)">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_formacao" class="form-control" name="ds_valores_formacao">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="!ehPIS">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.conforme_obra_civil" class="form-control" name="conforme_obra_civil">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_ajustes" class="form-control" name="ds_valores_ajustes">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                    ...
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" v-model="relatorioAnalise.ds_valores_observacoes" placeholder="Descreva a seguir as observações e ajustes realizados nos valores, bem como demais justificativas pertinentes." name="ds_valores_observacoes" v-resizable class="form-control ressaltar juntos"></textarea-autosize>
                    </div>
                  ...
                  <div class="box-body" v-if="!ehVoluntariado">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_liberacao_adiantamentos" class="form-control" name="ds_liberacao_adiantamentos">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                    ...
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" v-model="relatorioAnalise.ds_liberacao_observacoes" placeholder="Descreva a seguir, se for o caso, justificativa pela concessão de adiantamento." name="ds_liberacao_observacoes" v-resizable class="form-control ressaltar juntos"></textarea-autosize>
                    </div>
...
<!-- Prazo de Execução --------------------------------------------------------------------------------------------->
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_execucao_compatibilidade" class="form-control" name="ds_execucao_compatibilidade">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <input :readonly="somenteLeitura" v-model="relatorioAnalise.ds_execucao_prazo" name="ds_titulo" class="form-control" :disabled="true" id="ds_execucao_prazo" type="text" maxlength="255">
                        </td>
                      </tr>
                      <tr>
                        <td>Qual o prazo de vigência do projeto?</td>
                        <td>
                          <input :readonly="somenteLeitura" v-model="proposta.periodo_vigencia" name="ds_titulo" class="form-control" :disabled="true" id="prazo_vigencia" type="text" maxlength="255">
                        </td>
                      </tr>
                      </tbody>
                    </table>
                ...
                <!-- Periodicidade dos Relatórios -->
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td style="width: 145px;">
                          <input :readonly="somenteLeitura" v-model="relatorioAnalise.ds_execucao_relatorios" name="ds_execucao_relatorios" class="form-control" id="ds_execucao_relatorios" type="text" maxlength="255" :disabled="true">
                        </td>
                      </tr>
                      </tbody>
                    </table>
                    ...
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.ds_execucao_observacoes" placeholder="Informe observações relevantes referente ao prazo de execução do projeto." name="ds_execucao_observacoes" v-resizable class="form-control ressaltar juntos"></textarea-autosize>
                    </div>
                  ...
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_objetivos" class="form-control" name="ds_regularidade_objetivos">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_documentação" class="form-control" name="ds_regularidade_documentação">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_cadastral" class="form-control" name="ds_regularidade_cadastral">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="_.includes([2, 3], faixaDoProjeto.faixa) && !ehPIS">
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_fiscal" class="form-control" name="ds_regularidade_fiscal">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_projetos" class="form-control" name="ds_regularidade_projetos">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_ocorrencias" class="form-control" name="ds_regularidade_ocorrencias">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr v-if="!ehSimplificada">
                        ...
                        <td v-if="relatorioAnalise.ds_regularidade_rating">
                          <input :readonly="somenteLeitura" v-model="relatorioAnalise.ds_regularidade_rating.ds_descricao" name="ds_titulo" class="form-control" id="ds_regularidade_rating" type="text" maxlength="255" :disabled="true">
                        </td>
                      </tr>
                      <!-- Demanda 64783 - Inclusão do risco da proposta na visualização do relatório -->
                      <tr>
                  ...
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_licenciamento_conforme" class="form-control" name="ds_licenciamento_conforme">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <input :readonly="somenteLeitura" v-model="relatorioAnalise.ds_licenciamento_documentacao" name="ds_titulo" class="form-control" id="ds_licenciamento_documentacao" type="text" maxlength="255">
                        </td>
                      </tr>
                      </tbody>
                    </table>
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" v-model="relatorioAnalise.ds_licenciamento_observacoes" placeholder="Descreva qual a documentação apresentada, inexigibilidade ou dispensa, conforme o caso." name="ds_licenciamento_observacoes" v-resizable class="form-control ressaltar juntos"></textarea-autosize>
                    </div>
                  ...
                  <div class="box-body">
                    ...
                    <table class="table table-condensed table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_enquadramento_orcamento" class="form-control" name="ds_enquadramento_orcamento">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                      ...
                      <tr>
                        ...
                        <td>
                          <select :readonly="somenteLeitura" v-model="relatorioAnalise.ds_enquadramento_minuta" class="form-control" name="ds_enquadramento_minuta">
                            <option :value="opcao" v-for="opcao in opcoes">{{opcao}}</option>
                          </select>
                        </td>
                      </tr>
                    ...
                    <div class="nao_imprimir">
                      <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.ds_enquadramento_observacoes" placeholder="Informe o embasamento jurídico, parecer e/ou minuta padrão utilizados para amparo legal da minuta de convênio." name="ds_enquadramento_observacoes" v-resizable class="form-control ressaltar juntos"></textarea-autosize>
                    </div>
                  ...
                  <div class="box-body juntos" v-if="ehPIS || ehVoluntariado">
                    ...
                    <table class="bloco">
                      <tr>
                        <td>
                          <div class="nao_imprimir">
                            <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.ds_consideracoes_finais" rows="7" v-resizable name="ds_consideracoes_finais" class="form-control"></textarea-autosize>
                          </div>
                    ...
                    <div class="box-body">
                      ...
                      <table class="bloco">
                        <tr>
                          <td>
                            <div class="nao_imprimir">
                              <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.ds_parecer" rows="7" v-resizable name="ds_parecer" class="form-control"></textarea-autosize>
                            </div>
                    ...
                    <div class="box-body">
                      ...
                      <table class="bloco">
                        <tr>
                          <td>
                            <div class="nao_imprimir">
                              <textarea-autosize :readonly="somenteLeitura" @input="resizable" v-model="relatorioAnalise.ds_despacho_observacoes" rows="7" v-resizable name="ds_despacho_observacoes" class="form-control"></textarea-autosize>
                            </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        faixasInvestimento: [{faixa: 1, de: 0, para: 100000, alcada: 'Comitê de Desenvolvimento Social'}, {faixa: 2, de: 100000.01, para: 299999.99, alcada: 'Comitê de Desenvolvimento Social'}, {faixa: 3, de: 300000, para: 1000000000, alcada: 'Comitê Estratégico'}],
        ehPIS: false,
    ...
    methods: {
      async carregarRelatorio () {
        ...
        this.mostrarLogCarga = true

        try { await this.buscaPublicoAlvo() } catch (e) { console.error('Erro em buscaPublicoAlvo', e) }
        try { await this.buscarBaseODS() } catch (e) { console.error('Erro em buscarBaseODS', e) }
        try { await this.consultaTecnologiasCadastradas() } catch (e) { console.error('Erro em consultaTecnologiasCadastradas', e) }
        try { await this.buscaDefinicaoPublicoAlvo() } catch (e) { console.error('Erro em buscaDefinicaoPublicoAlvo', e) }
        try { await this.buscaIndicadorAvaliacao() } catch (e) { console.error('Erro em buscaIndicadorAvaliacao', e) }
        try { await this.buscaPropostaSimplificada() } catch (e) { console.error('Erro em buscaPropostaSimplificada', e) }
        try { await this.buscaProposta() } catch (e) { console.error('Erro em buscaProposta', e) }
        try { await this.carregarProjetoDuplicador() } catch (e) { console.error('Erro em carregarProjetoDuplicador', e) }
        try { await this.buscaEntidades() } catch (e) { console.error('Erro em buscaEntidades', e) }

        if (this.verificaErro()) {
          this.mostrarLogCarga = true
          this.mostrarAviso = true
        } else {
          this.mostrarLogCarga = false
        }
      },
      closewindow () {
        close()
      },
      gerenciaRequisicao (nome, status, erro = null) {
        const existente = this.etapaCarga.find(item => item.nome === nome)
        if (existente) {
          ...
          this.etapaCarga.push({ nome, status })
        }
      },
      verificaErro () {
        return this.etapaCarga.some(objeto => objeto.status === 'ERRO')
      },
      recarregarRelatorio () {
      ...
      async buscaProposta () {
        this.gerenciaRequisicao('Relatório de Análise', 'Carregando')
        this.loading.show = true
        // Demanda 66677 - Correção na chamada da função de cálculo de risco
        // Suporte 69.458 - correção na chamada da API
        // await axiosLaravel.get('projeto/numero/' + this.$route.params.id).then(async (response) => {
        await axiosLaravel.get('propostas/' + this.$route.params.id).then(async (response) => {
          ...
          this.$forceUpdate()
          this.gerenciaRequisicao('Relatório de Análise', 'OK')
          this.loading.show = false
        })
        .catch((error) => {
          this.gerenciaRequisicao('Relatório de Análise', 'ERRO', error)
          this.loading.show = false
      ...
      carregarProjetoDuplicador () {
        this.gerenciaRequisicao('Projeto Duplicador', 'Carregando')
        axiosLaravel.get('projeto/sapiens/' + this.proposta.nr_projeto_duplicacao).then((response) => {
          ...
          this.gerenciaRequisicao('Projeto Duplicador', 'OK')
        })
          .catch((error) => {
            this.$snotify.clear()
            this.gerenciaRequisicao('Projeto Duplicador', 'ERRO', error)
            this.$root.trataErro(error)
          })
      },
      async carregarProjetoSuperior () {
        this.gerenciaRequisicao('Projeto Superior', 'Carregando')
        await axiosLaravel.get('projeto/sapiens/' + this.proposta.duplicadorSapiens.prjsup).then((response) => {
          ...
          this.gerenciaRequisicao('Projeto Superior', 'OK')
        })
          .catch((error) => {
            this.gerenciaRequisicao('Projeto Superior', 'ERRO', error)
            this.$snotify.clear()
            this.$root.trataErro(error)
          })
      },
      async buscarBaseODS () {
        this.loading.show = true
        this.gerenciaRequisicao('ODS', 'Carregando')
        await axiosLaravel.get('/ods').then((response) => {
          ...
          .catch((error) => {
            // this.loading.show = false
            this.gerenciaRequisicao('ODS', 'ERRO', error)
            this.$root.trataErro(error)
          })
      },
      async buscaPropostaSimplificada () {
        // Criamos esse controle pra não perder a referencia do modelo das propostas antigas
        this.gerenciaRequisicao('Ver. Prop. Simplificada', 'Carregando')
        this.loading.show = true
        console.log('BUSCA PROPOSTA SIMPLIFICADA')
        await axiosLaravel.get('/simplificado/verificaPropostaSimplificada/' + this.$route.params.id).then((response) => {
          this.ehSimplificada = response.data.simplificado === 1
          console.log('ehSimplificada: ', this.ehSimplificada)
          this.$forceUpdate()
          this.gerenciaRequisicao('Ver. Prop. Simplificada', 'OK')
        })
          .catch((error) => {
            this.gerenciaRequisicao('Ver. Prop. Simplificada', 'ERRO', error)
            this.loading.show = false
            this.$root.trataErro(error)
          })
      },
      ...
      async carregarODS () {
        await axiosLaravel.get('/ods/getMetasODS/' + this.$route.params.id).then((response) => {
          ...
          this.gerenciaRequisicao('ODS', 'OK')
        })
          .catch((error) => {
            this.gerenciaRequisicao('ODS', 'ERRO', error)
            this.$snotify.clear()
      ...
      async consultaTecnologiasCadastradas () {
        this.loading.show = true
        this.gerenciaRequisicao('Tecnologias Sociais', 'Carregando')
        await axiosLaravel.get('/tecnologiasocial/consultarTecnologiaPorProposta/' + this.$route.params.id).then((response) => {
          ...
          // this.loading.show = false
          this.gerenciaRequisicao('Tecnologias Sociais', 'OK')
          this.$forceUpdate()
        })
        .catch((error) => {
          this.gerenciaRequisicao('Tecnologias Sociais', 'ERRO', error)
          this.$root.trataErro(error)
          this.loading.show = false
          this.$forceUpdate()
        })
      ...
      async buscaDefinicaoPublicoAlvo () {
        this.loading.show = true
        this.gerenciaRequisicao('Público-Alvo Previsto', 'Carregando')
        ...
        .catch((error) => {
          this.gerenciaRequisicao('Público-Alvo Previsto', 'ERRO', error)
          this.$root.trataErro(error)
          this.loading.show = false
        })
      },
      async buscaPublicoAlvoPrevisto () {
        ...
        await axiosLaravel.get('/definicaoparticipante/getPrevistosProjeto/' + this.$route.params.id).then((response) => {
          ...
          this.gerenciaRequisicao('Público-Alvo Previsto', 'OK')
          this.$forceUpdate()
        })
        .catch((error) => {
          this.gerenciaRequisicao('Público-Alvo Previsto', 'ERRO', error)
      ...
      async buscaIndicadorAvaliacao () {
        this.loading.show = true
        this.gerenciaRequisicao('Indicadores de Avaliação', 'Carregando')
        await axiosLaravel.get('/avaliacao/carregarRespostasRelatorioAnalise/' + this.$route.params.id).then((response) => {
          ...
          // this.loading.show = false
          this.gerenciaRequisicao('Indicadores de Avaliação', 'OK')
          this.$forceUpdate()
        })
        .catch((error) => {
          this.gerenciaRequisicao('Indicadores de Avaliação', 'ERRO', error)
          this.$root.trataErro(error)
          this.loading.show = false
          this.$forceUpdate()
        })
      ...
      async buscaEntidades () {
        ...
        await axiosLaravel.get('entidades/' + this.proposta.entidade.id).then((response) => {
          ...
          this.gerenciaRequisicao('Entidade', 'OK')
          this.proposta.entidade.dirigentes = array
          this.$forceUpdate
        })
          .catch((error) => {
            this.gerenciaRequisicao('Entidade', 'ERRO', error)
            console.error(error)
            if (error) {
              this.$snotify.clear()
              this.$root.trataErro(error)
      ...
      async buscaPublicoAlvo () {
        this.gerenciaRequisicao('Público-Alvo Fonte', 'Carregando')
        await axiosLaravel.get('publico-alvo-fonte').then((response) => {
          this.publicosAlvo = response.data
          this.gerenciaRequisicao('Público-Alvo Fonte', 'OK')
        })
          .catch((error) => {
            this.gerenciaRequisicao('Público-Alvo Fonte', 'ERRO', error)
            this.$root.trataErro(error)
          })
      },
      buscaDadosProposta () {
      ...
      atualizaProposta () {
        ...
        // console.log('Aberto Broadcast ' + 'proposta-' + this.$route.params.id)
...
<style scoped>
  ...
  .etapa-carga-content {
    ...
    overflow: auto;
    animation: appear-etapa 0.5s ease;
  }
  ...
  .etapa-carga-log-button:hover {
    scale: 1.1;
  }

  .etapa-close-container {
    width: 100%;
  }
  ...
  .etapa-close-button:hover {
    scale: 1.1;
  }

  .etapa-aviso-container {
    width: 60%;
    background-color: #efd50e;
    margin-bottom: 10px;
  ...
  .etapa-button-reload:hover {
    scale: 0.98;
  }

  @keyframes appear-etapa {
  0% {
    opacity: 0;
