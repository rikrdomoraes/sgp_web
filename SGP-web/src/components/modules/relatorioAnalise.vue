<template>
  ...
  <div class="content-wrapper" style="min-height: 870px;">
    ...
    <div v-if="itemSemContaFinanceiraFBB || itemSemContaFinanceiraParceiro || semProjetoDuplicador" class="column centered-start etapa-aviso-conta-container-geral">
      <div v-if="itemSemContaFinanceiraFBB || itemSemContaFinanceiraParceiro" class="etapa-aviso-conta-container">
        <i class="fa fa-warning"></i> Atenção: Existem itens de orçamento sem conta financeira cadastrada. Favor Verificar.
      </div>
      <div v-if="semProjetoDuplicador" class="etapa-aviso-conta-container">
        <i class="fa fa-warning"></i> Atenção: Projeto de Duplicação ainda não foi selecionado. Favor verificar.
      </div>
    </div>    
    <div class="column centered-start etapa-carga-container" v-if="mostrarLogCarga">
      ...
      <div class="column centered-start etapa-carga-content">
        ...
        <div class="row-flex centered-start etapa-carga-titulo"><i class="fa fa-user-secret"></i> Status de Carga da Edição do Relatório de Análise</div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        mostrarAviso: false,
        riscoCalculado: '',
        itemSemContaFinanceiraFBB: false,
        itemSemContaFinanceiraParceiro: false,
        semProjetoDuplicador: false,
        showgrid: true,
    ...
    methods: {
      async carregarRelatorio () {
        ...
        try { await this.consultaRiscoCalculado() } catch (e) { console.error('Erro em consultaRiscoCalculado', e) }
        try { await this.buscaProposta() } catch (e) { console.error('Erro em buscaProposta', e) }
        try { await this.carregaRating() } catch (e) { console.error('Erro em carregaRating', e) }
      ...
      async buscaProposta () {
        ...
        await axiosLaravel.get('propostas/' + this.$route.params.id).then(async (response) => {
          ...
          let itemSemContaFBB = false
          let agrupadoContasFBB = _.groupBy(itemFBB, function (g) {
            if (g.conta_financeira) {
              return g.conta_financeira.descricao
            } else {
              if (!itemSemContaFBB) {
                itemSemContaFBB = true
              }
              return 'SEM CONTA FINANCEIRA CADASTRADA'
            }
          })
          this.itemSemContaFinanceiraFBB = itemSemContaFBB
          let contasValorFBB = []
          ...
          //
          let itemSemContaParceiro = false
          let agrupadoContasParcerias = _.groupBy(itemParceria, function (g) {
            if (g.conta_financeira) {
              return g.conta_financeira.descricao
            } else {
              if (!itemSemContaParceiro) {
                itemSemContaParceiro = true
              }
              return 'SEM CONTA FINANCEIRA CADASTRADA'
            }
          })
          this.itemSemContaFinanceiraParceiro = itemSemContaParceiro
      ...
      async carregarProjetoDuplicador () {
        if (!this.proposta.nr_projeto_duplicacao) {
          this.semProjetoDuplicador = true
          this.gerenciaRequisicao('Projeto Duplicador', 'NAO CADASTRADO')
          return
        }
      ...
      async carregaRating () {
        this.gerenciaRequisicao('Rating', 'Carregando')
        await axiosLaravel.post('rating/carregar/', {'idProposta': this.proposta.id}).then(async (response) => {
          if (!_.isNil(response.data) && !_.isEmpty(response.data)) {
            _.set(this.proposta, 'rating', response.data)
          }
          this.$forceUpdate()
          this.gerenciaRequisicao('Rating', 'OK')
        })
          .catch((error) => {
            this.gerenciaRequisicao('Rating', 'ERRO', error)
            this.$root.trataErro(error)
            console.log('ERRO: ', error)
          })
      },
      ...
      consultaRiscoCalculado () {
        this.gerenciaRequisicao('Risco Calculado', 'Carregando')
        axiosLaravel.get('propostas/' + this.$route.params.id + '/consulta-risco-calculado').then(async (response) => {
          this.riscoCalculado = response.data
          this.gerenciaRequisicao('Risco Calculado', 'OK')
          this.$forceUpdate()
        })
         .catch((error) => {
           this.gerenciaRequisicao('Risco Calculado', 'ERRO', error)
           this.$root.trataErro(error)
           console.log('ERRO: ', error)
         })
      }
...
<style scoped>
  ...
  .etapa-aviso-conta-container-geral {
    width: 99%;
    margin-top: 150px;
    margin-bottom: -100px;
    gap: 5px;
