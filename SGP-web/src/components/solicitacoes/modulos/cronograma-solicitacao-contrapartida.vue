        ...
        <div class="col-md-12 financial-physical-schedule">
          <table class="table table-condensed table-bordered">
            ...
            <tr>
              ...
              <th v-if="propsEtapa !== 'Solicitação'">Economicidade</th>
            ...
            <tr v-for="item in itensProjeto">
              <!--Item-->
              <td>{{item.Item}} - {{item.Item_Descricao}}<br>#{{item.Id_item}} {{item.Item_Observacao}}</td>
              <!--Tipo-->
              ...
              <td  class="text-center" v-if="propsEtapa !== 'Solicitação'">
                <label class="checkbox-inline new-control new-control-checkbox" style="margin-bottom: 10px;">
                  <input :disabled="propsEtapa !== 'Análise' || item.editing === false || _.isUndefined(item.editing)" type="checkbox"  v-model="item.Economicidade" @change="marcaItem(item)">
                  <div class="new-control-indicator"></div>
                </label>
              </td>
              <!--Quantidade a comprovar-->
              <td v-if="propsEtapa !== 'Solicitação'" class="text-center">{{item.Qtde_saldo}}</td>
              ...
              <!--Valor a comprovar-->
              <td v-if="propsEtapa !== 'Solicitação'" class="text-center">{{item.Valor_saldo | real }}</td>
              ...
              <td class="text-center">
                <i v-if="item.editing === false || !item.editing" v-show="propsEtapa !== 'Autorização' && item.Valor_saldo > 0" class="fa fa-edit" @click.prevent="editItem(item, 1)" style="margin-left:5px"></i>
                <i v-if="item.editing === false && propsEtapa === 'Solicitação' && item.quantidadeSolicitada > 0" class="fa fa-times" @click.prevent="apagaItem(item)"   style="color: red;margin-left:5px; cursor: pointer"></i>
            ...
            <tr >
              <td colspan="5" style="border-right: none;">Valor total de recursos contratados: </td>
              <td colspan="4" class="text-right" style="border-left: none;">{{totais.valor_total_contratado | real}}</td>
              <td></td>
            </tr>
            <tr >
              <td colspan="5" bgcolor="#F0F8FF"  style="border-right: none;">Valor total da solicitação:	</td>
              <td colspan="4" bgcolor="#F0F8FF"  class="text-right" style="border-left: none;">{{total | real}}</td>
              <td></td>
            </tr>
            <tr >
              <td colspan="5" style="border-right: none;">Saldo restante do projeto:	</td>
...
<script>
  export default {
    name: 'cronograma-solicitacao-pagamento',
    data () {
      return {
    ...
    computed: {
      // codeItemsNota () {
      // return this.propsProjeto.projectItems.map(item => {
    ...
    methods: {
      ...
      editItem (item, parm) {
        ...
        if (this.propsEtapa === 'Solicitação') {
          ...
          if (item.valorSolicitado > item.Valor_saldo) {
            this.$snotify.error('Valor solicitado maior que o disponível!')
            item.valorSolicitado = 0
          }
          if (item.quantidadeSolicitada === 0 || item.quantidadeSolicitada === '0') {
            this.$snotify.error('Quantidade solicitada tem que ser maior que zero!')
            return
          }
          if (item.valorSolicitado === 0) {
        ...
        if (!item.editing) {
          ...
          if (this.propsNota.receiptItems.indexOf(item) === -1) this.propsNota.receiptItems.push(item) // Se o item não esta no receiptItems adiciona
        }
      ...
      apagaItem (item) {
        ...
        console.log(this.propsNota.receiptItems)
        let pos = this.propsNota.receiptItems.indexOf(item)
        console.log(this.propsNota.receiptItems.indexOf(item))
        this.propsNota.receiptItems.splice(pos, 1)
        console.log(this.propsNota.receiptItems)
        this.valorTotalSolicitacao()
      },
      buscaItensProjetoBackEnd () {
      ...
      marcaItem (item) {
        this.marcado = 1
        if (item.Economicidade === true) {
          item.Economicidade = 1
        } else {
          item.Economicidade = 0
        }
      },
      buscaRecursos () {
        this.totalFBB = 0
        ...
        this.saldoFasePAR = 0
        let idProposta = _.isUndefined(this.propsProjeto.Id_proposta) ? this.propsProjeto.fk_proposta : this.propsProjeto.Id_proposta
        axiosLaravel.get('processos/adiantamento/recursos/' + idProposta).then((response) => {
          ...
          for (i = 0; i < this.itensProjeto.length; i++) {
            for (j = 0; j < this.propsNota.itens_liberados.length; j++) {
              if (this.propsNota.itens_liberados[j].itens_orcamentos[0].fk_itens === this.itensProjeto[i].Item) {
                this.itensProjeto[i].quantidadeSolicitada = this.itensProjeto[i].Qtde_solicitado
                this.itensProjeto[i].valorSolicitado = this.itensProjeto[i].Valor_solicitado
              }
            }
          }
        }
