<template>
  <div class="box-body" style="overflow: auto" >
    ...
    <div v-show="mostrarOpcoes" class="row">
      <div class="col-md-12">
        ...
        <div class="quadro_opcoes" v-bind:style="'min-height: ' + ((atividades.length * 30 ) + 20) + 'px;'">
          ...
          <div>
            <button @click="carregaAtividades" :title="'Clique para trazer os dados salvos até o momento'" v-tippy="{ inertia : true, arrow : true,  animation : 'fade', distance:'1', size: 'large'}" class="btn btn-primary btn-flat"><i :class="'fa fa-refresh'"></i> Atualizar o gráfico</button>
          </div>
    ...
    <div>
      <div class="col-md-12 gantt_atividades">
        <div>
          ...
          <div class="cabecalho">
            <div class="linha_marcadoras" v-bind:style="'height: ' + ((atividades.length * 30 ) + 30) + 'px;'">
              ...
              <div class="hoje" v-bind:style="'margin-left: ' + (parseInt(tamanho_nome_atividade) + ((projeto.hoje_dias < ((projeto.periodos * 30) + 30) ? projeto.hoje_dias : ((projeto.periodos * 30) + 30))) * zoom) + 'px; height: ' + ((atividades.length * 30 ) + 60) + 'px; padding: ' + ((atividades.length * 30 ) + 40) + 'px 10px 0px 5px;'">
                <span :title="(dia_noite === 'DIA' ? 'Bom dia!' : dia_noite === 'TARDE' ? 'Boa Tarde!' : 'Boa noite!') + '<br/> Já se passaram <b>' + projeto.hoje_dias + ' dias' + '</b> do início do projeto'"
        ...
        <div v-show="loadingShow" class="overlay" style="position: absolute; top: 0px; left: 0px; width: 100%; height: 100%; display: block; z-index: 1000">
...
<script>
  export default {
    ...
    props: ['idProjeto', 'tipoBarra', 'mostrarExecFinanc', 'mostrarExecDeclarada'],
    watch: {
    },
    computed: {
    },
    ...
    methods: {
      carregaAtividades () {
        ...
        if (this.$route.params.idProjeto) {
          idProjeto = this.$route.params.idProjeto
        }
        if (this.$route.params.id) {
          idProjeto = this.$route.params.id
        }
        if (this.$route.params.numero_projeto) {
          idProjeto = this.$route.params.numero_projeto
        }
        this.loadingShow = true
        axiosLaravel.get('atividade/gantt/' + idProjeto).then((response) => {
