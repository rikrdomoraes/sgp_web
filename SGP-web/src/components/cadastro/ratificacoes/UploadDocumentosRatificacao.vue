...
<!---->
        <!-- buttons -->
        <div v-if="! foiAssinada" class="form-group" style="margin-bottom:0">
    ...
    <div v-if="showGrid">
      ...
      <div class="row" v-if="!disable">
        <div class="col-sm-12">
          <button :disabled="foiAssinada" if="permite_edicao" @click="novoDocumento" type="button" class="btn btn-primary btn-flat"><i class="fa fa-plus-circle" aria-hidden="true"></i> Novo documento</button>
        </div>
        <!-- /.col -->
      </div>
      <!-- /.row -->
          ...
          <div class="table-responsive">
            <table class="table table-condensed table-bordered table-striped" >
              ...
              <tr v-for="(documento, indexDocumento) in propDocumentos"  style="text-align: center">
                ...
                <td v-if="!disable">
                  <button :disabled="foiAssinada" v-if="!disable" type="button" v-confirm="{ok: retorno => deletaDocumento(indexDocumento, documento), cancel: doNothing,
...
<script>
  ...
  export default {
    name: 'upload-documentos-ratificacao',
    data () {
      return {
        showGrid: true,
    ...
    props: ['propDocumentos', 'documentTypes', 'documentosObrigatorios', 'disable', 'localUso', 'processo_ratificacao', 'foiAssinada'],
    created () {
    ...
    methods: {
      ...
      deletaDocumento (indexDocumento, documento) {
        console.log('this.processo_ratificacao: ', this.processo_ratificacao)
        console.log('documento: ', documento)
        axiosLaravel.post('/ratificacao/deleta_documento', {'documento_id': documento.id, 'processo_id': this.processo_ratificacao}).then((response) => {
          if (response.data.codigo === 'OK') {
            console.log('retorno do deletaDocumento', response.data.retorno)
