        ...
        <div class="row">
          <div class="col-md-12">
            <div class="ng-scope ng-isolate-scope">
                  ...
                  <fieldset :disabled="this.$router.currentRoute.name === 'proposta-visualiza'" >
                    <div class="form-horizontal ng-valid-maxlength ng-invalid ng-invalid-required ng-valid-date ng-invalid-min ng-dirty ng-valid-mask ng-valid-parse">
                            ...
                            <div class="row">
                              ...
                              <div class="col-md-7 col-md-offset-1">
                                <div class="col-md-6">
                                  ...
                                  <!--Data inicial-->
                                  <div ng-if="regComProFormCtrl.isAval('INITIALDATE')" class=" col-md-12 ng-scope required-field" style="margin-left: -15px"
                      ...
                      <div class="col-md-12" v-show="mostraCronograma">
                        ...
                        <cronograma :proposta="proposta" :propsObjetivos="proposta.objectives"
                                    :atividades="atividades" :meses="proposta.periods"
                                    :projeto-duplicacao="proposta.duplicateProject"
                                    v-on:itemAdicionado="atualizaCronograma"></cronograma>
                      </div>
                      ...
                      <div class="col-md-12">
                        ...
                        <parcela-adiantamento :meses="proposta.periods" @metaAdicionada="proposta.objectives = $event"></parcela-adiantamento>
                      </div>
                      ...
                      <div class="col-md-12">
                        ...
                        <historico-proposta :proposta="proposta" :disabled="this.$router.currentRoute.name === 'proposta-visualiza'"></historico-proposta>
                      </div>
             ...
             <analise :propsAnalise="analise"></analise>
            </div>
          </div>
        </div>
      </section>
  ...
  export default {
    ...
    methods: {
      buscaAnalise () {
        ...
        axios.get('proxy/project/approval-analysis/' + this.$route.params.id).then((response) => {
      ...
      calculatePlots () {
        ...
        for (let i = 0; i < this.atividades.length; i++) {
          for (let j = 0; j < this.atividades[i].items.length; j++) {
            if (this.atividades[i].items[j].foreseeable) {
              items.push(this.atividades[i].items[j])
            }
          }
        }
        ...
        for (let k = 0; k < items.length; k++) {
          ...
          if (item.resourceSource === 'CONTRAPARTIDA') {
            this.calcPlotItemValue(data, item.totalValue, item.finalPeriod)
          } else if (item.type === 'PRODUTO') {
            this.calcPlotItemValue(data, item.totalValue, item.startPeriod)
          } else {
            let parcelas = item.finalPeriod - item.startPeriod + 1

            let vlrParcelas = item.totalValue / parcelas

            vlrParcelas *= 100

            let vlrScale = parseInt(vlrParcelas)

            vlrParcelas = vlrScale / 100

            let vlrResto = item.totalValue - (vlrParcelas * parcelas)

            for (let l = item.startPeriod; l <= item.finalPeriod; l++) {
              if (l === item.finalPeriod) {
                vlrParcelas += vlrResto
              }
              this.calcPlotItemValue(data, vlrParcelas, l)
            }
          }
        }
        data.sort((a, b) => {
        ...
        this.proposta.advancePlots = data
        this.$forceUpdate()
      },
      calcPlotItemValue (data, totalValue, period) {
